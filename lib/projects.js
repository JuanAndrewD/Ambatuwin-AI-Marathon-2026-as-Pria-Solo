...
function newDocument({ type = 'note', name = 'Untitled.md', content = '', included_in_context = false, pinned = false, binary = false, mime = '', orig_bytes = 0 } = {}) {
  return {
    ...
    included_in_context,
    pinned,
    binary: !!binary,        // unstructured/binary docs: preview-only, never editable
    mime: mime || '',        // original MIME type (binary docs only)
    orig_bytes: orig_bytes || 0, // size of the original file (binary docs only)
    created_at: nowISO(),
    updated_at: nowISO(),
  };
}
...
async function listProjects(userId) {
  ...
  return rows.map(rowToProject).map(p => ({
    ...
    has_repo: !!p.repo,
    repo_full_name: p.repo?.full_name || null,
  }));
}
...
// Attach (or clear) the connected repository mapping for a project. At most one
// repo per project; pass null to detach. Scoped to the owner so another account
// cannot map a repo onto a project that isn't theirs.
async function setRepo(userId, id, repo) {
  const { rows } = await db.query(
    `UPDATE projects SET repo = $3::jsonb, updated_at = now()
     WHERE id = $1 AND user_id = $2 RETURNING *`,
    [id, userId, repo ? JSON.stringify(repo) : null]
  );
  return rowToProject(rows[0]);
}
...
async function listDocuments(userId, projectId) {
  ...
  return p.documents.map(d => ({
    ...
    type: d.type,
    name: d.name,
    bytes: d.binary ? (d.orig_bytes || 0) : Buffer.byteLength(d.content || '', 'utf8'),
    text_bytes: Buffer.byteLength(d.content || '', 'utf8'),
...
async function createDocument(userId, projectId, payload) {
  ...
  const doc = newDocument({
    ...
    mime: payload.mime || '',
    orig_bytes: payload.orig_bytes || 0,
  });
...
async function updateDocument(userId, projectId, docId, patch) {
  ...
  if (!doc) return null;
  // Binary/unstructured docs are preview-only: never allow content/type edits,
  // only renaming and context-toggle.
  const allowed = doc.binary
...
async function deleteDocument(userId, projectId, docId) {
  ...
  const wasBinary = p.documents[idx].binary;
  p.documents.splice(idx, 1);
  await persist(p);
  if (wasBinary) {
    try { await db.query('DELETE FROM document_blobs WHERE project_id = $1 AND doc_id = $2', [projectId, docId]); }
    catch (err) { console.error('[projects] blob cleanup failed:', err.message); }
  }
  return true;
}

// ---- Binary document blobs -------------------------------------------------
// Original bytes for unstructured docs live in their own table so they never
// bloat the frequently-read project row. Ownership is enforced by first
// confirming the caller owns the project.

async function putDocumentBlob(userId, projectId, docId, { buffer, mime }) {
  const p = await loadOwned(userId, projectId);
  if (!p) return false;
  if (!p.documents.find(d => d.id === docId)) return false;
  await db.query(
    `INSERT INTO document_blobs (project_id, doc_id, mime, data, bytes)
     VALUES ($1, $2, $3, $4, $5)
     ON CONFLICT (project_id, doc_id)
       DO UPDATE SET mime = EXCLUDED.mime, data = EXCLUDED.data, bytes = EXCLUDED.bytes`,
    [projectId, docId, mime || '', buffer, buffer.length]
  );
  return true;
}

async function getDocumentBlob(userId, projectId, docId) {
  const p = await loadOwned(userId, projectId);
  if (!p) return null;
  const { rows } = await db.query(
    'SELECT mime, data, bytes FROM document_blobs WHERE project_id = $1 AND doc_id = $2',
    [projectId, docId]
  );
  if (!rows[0]) return null;
  return { mime: rows[0].mime || '', data: rows[0].data, bytes: Number(rows[0].bytes) };
}
