...
function init() {
  if (!initPromise) {
    initPromise = (async () => {
      // Guard against a non-UTF-8 database. Unstructured docs (PDF/DOCX/PPTX)
      // and LLM output routinely contain characters (✅ ● — “smart quotes”, CJK,
      // emoji) that WIN1252/LATIN1 cannot store, which makes writes fail at
      // runtime with 'has no equivalent in encoding'. Fail fast with a clear,
      // actionable message instead.
      try {
        const enc = await pool.query('SHOW server_encoding');
        const encoding = String(enc.rows[0]?.server_encoding || '').toUpperCase();
        if (encoding && encoding !== 'UTF8' && encoding !== 'UTF-8') {
          throw new Error(
            `Database server_encoding is ${encoding}, but this app requires UTF8. ` +
            `Create a UTF-8 database and point DATABASE_URL at it, e.g.:\n` +
            `  CREATE DATABASE cia WITH ENCODING 'UTF8' LC_COLLATE 'C' LC_CTYPE 'C' TEMPLATE template0;`
          );
        }
      } catch (err) {
        if (/requires UTF8/.test(err.message)) throw err;
        // If the probe itself failed (e.g. permissions), don't block startup.
        console.warn('[db] could not verify server_encoding:', err.message);
      }
