...
const ARCH_SYSTEM = (regionCode, regionMeta) => `You are a senior AWS Solutions Architect. You will design a production-grade architecture.

You MUST only use these AWS services (exact spelling): ${ALLOWED_SERVICES.join(', ')}.
...
function validateAndPrice(arch, regionCode) {
  const issues = [];
  const items = [];
  let total = 0;

  for (const c of arch.components || []) {
    if (!ALLOWED_SERVICES.includes(c.service)) {
...
async function design({ brief, regionCode, additionalCompliance = [] }) {
  ...
  const archRes = await chat([
    { role: 'system', content: ARCH_SYSTEM(regionCode, regionMeta) },
  ...
  const priced = validateAndPrice(arch, regionCode);
