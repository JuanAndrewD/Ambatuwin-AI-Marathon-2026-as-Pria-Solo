...
const CHAT_SYSTEM = (project, catalogPublic) => `You are the Cloud Infrastructure Architect — a senior AWS Solutions Architect acting as a Technical Sales Consultant. You are advising on project "${project.name}".
...
- Current brief on file: """${project.brief || '(empty — ask the user to provide one)'}"""
...
- When the user asks for an architecture, suggest using the "Generate plan" Studio action so the deterministic pricing engine runs.
- If the user wants to change the project's brief, region, or enabled services, tell them which sidebar control to use.
...
async function chatTurn({ project, history = [], userMessage, catalogPublic }) {
  if (!userMessage || !userMessage.trim()) throw new Error('Empty message');
  const messages = [
    { role: 'system', content: CHAT_SYSTEM(project, catalogPublic) },
...
module.exports = { design, chatTurn, ALL_SERVICES };
